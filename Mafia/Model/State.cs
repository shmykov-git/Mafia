...
public class State
{
    ...
    public required List<OperationResult> Processes { get; set; }
    public OperationResult CurrentProcess => Processes[^1];
    ...
    public Player[] CurrentKills() => CurrentProcess.AllKills();
    ...
    public bool IsSelfSelected(Player player) => Processes.Select(ps => ps).Any(ops => ops.Selects?.Any(s => s.Who == player && s.Whom.Contains(player)) ?? false);
