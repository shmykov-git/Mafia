...
{
    public static async Task<DailyNews> CityBan(State state, CityAction action)
    {
        var dailyNews = new DailyNews
        {
            Selects = [new Select { Operation = nameof(CityBan), Who = null!, UserWhom = await state.Host.AskCityToSelect(state, action, nameof(CityBan)) }]
        };
